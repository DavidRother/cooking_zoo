...
class CookingEnvironment(AECEnv):
    ...
    def observe(self, agent):
        ...
        agent_location = np.asarray(self.world_agent_mapping[agent], np.int32)
        observation = {'symbolic_observation': tensor_obs,
                       'agent_location': agent_location,
                       'goal_vector': self.recipe_mapping[agent].goals_completed}
        return observation
    ...
    def create_observation(self, agent_num, open_goals, tensor_obs):
        agent_location = np.asarray(self.sim_agents[agent_num].location, np.int32)
        observation = {'symbolic_observation': tensor_obs,
                       'agent_location': agent_location,
